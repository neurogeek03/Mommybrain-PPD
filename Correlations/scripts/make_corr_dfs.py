...
out_path = project_dir / 'out' / 'update'
    ...
    for dir in seq_dir.iterdir():
        for seq_file in dir.glob("*.tsv"):
            if re.match(celltype, seq_file.stem):
                ...
                seq_df = pd.read_csv(seq_filepath, sep="\t", index_col=0)
                #seq_df.columns = ['gene','logFC','logCPM','LR','PValue','FDR']
                print('Merging...')
                merged= pd.merge(seq_df, tags_df, left_index=True, right_index=True, how='inner')
                ...
                if not filtered.empty:
                    filtered_path = comparison_dir / f'filtered_{celltype}_tags_seq_merged.tsv'
