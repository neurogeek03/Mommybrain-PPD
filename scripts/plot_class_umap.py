...
adata.obs['allen_class'] = adata.obs[obs_column].map(nn_subclass_to_class)
...
# Subset AnnData
adata_filtered = adata[adata.obs[color_col].isin(types_to_keep)].copy()
...
# umap_df["label_number"] = umap_df[color_col].str.split("_").str[0].astype(int)
# ordered_labels = umap_df.groupby(color_col, observed=True)["label_number"].first().sort_values().index.tolist()
# umap_df[color_col] = pd.Categorical(umap_df[color_col], categories=ordered_labels, ordered=True)

umap_df = umap_df.sort_values(color_col)
umap_df.head()
...
fig = px.scatter(
        ...
        y="UMAP2",
        color=color_col,
        color_discrete_map=cluster_colors,  # <-- use your palette
        hover_data={
...
fig_path = os.path.join(out_dir, f"new_{color_col}_BrainCanada_Slide-seq_umap_interactive.html")
